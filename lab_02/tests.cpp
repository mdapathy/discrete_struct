...
TEST_CASE("Proper order of nodes in an array")
{
	Tree tr;
	tr.push(20);
	tr.push(21);
	...
	CHECK((arr[0]) == tr.get_root());
	CHECK(arr[1]->value == 0); //non-existing elemets are not to be changed 
	CHECK(arr[2] == tr.get_root()->right); 

};

TEST_CASE("Finding maximum leaf")
{
	Tree tr;
	tr.push(15);//maximum element
	tr.push(13);
	tr.push(14); // maximum leaf

	CHECK(tr.find_max_leaf() == 14);
};

TEST_CASE("Finding maximum leaf")
{
	Tree tr;
	tr.push(12);//min element
	tr.push(13);
	tr.push(14);//min leaf

	CHECK(tr.find_min_leaf() == 14);
};
